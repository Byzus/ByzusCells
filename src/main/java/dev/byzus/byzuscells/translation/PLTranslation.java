...
public class PLTranslation {
    ...
    public Component PL_CELL_DELETED = Components.success("Pomyślnie usunięto celę");
    public Component PL_PLAYER_ADDED_TO_CELL = Components.success("Pomyślnie dodano gracza do celi.");
    public Component PL_CREATED_CELL = Components.success("Pomyślnie utworzono celę o numerze: {cellID}");
    public Component PL_PLAYER_REMOVED_FROM_CELL = Components.success("Pomyślnie usunięto gracza z celi.");
    public Component PL_CANNOT_FIND_PLAYER = Components.error("Nie można znaleźć gracza o nicku: {target}");
